# ============================================================
# Script: sanitary_visit_mr_upstream_mines_ols_ioc.r
# Purpose: OLS of sanitary visit (binary) on an MR violation onset binary,
#          an above-median-upstream-mines binary, and a formal enforcement
...
#          Col 4: year + CWS FE. Unlike
#          sanitary_visit_mr_upstream_mines_ols.r, here BOTH mr_violation
#          and any_formal are restricted to IOC contaminants only — nitrate
#          (RULE_CODE 331), arsenic (332), inorganic chemicals (333) — so
#          the sample of underlying violations/enforcement actions feeding
#          each binary is IOC-specific rather than all contaminants.
#          MR violation onset defined as in
...
# Outputs: output/reg/sanitary_visit_mr_upstream_mines_ols_ioc.tex
...
# same convention as regulator_response_by_viol_type_main_states.r.
...
# ── 3. IOC violations/enforcement: RULE_CODE in {331, 332, 333} ─────────────
ve <- fread(file.path(SDWA_DIR, "SDWA_VIOLATIONS_ENFORCEMENT.csv"),
            select = c("PWSID", "VIOLATION_ID", "COMPL_PER_BEGIN_DATE",
                       "NON_COMPL_PER_BEGIN_DATE", "VIOLATION_CATEGORY_CODE",
                       "RULE_CODE", "ENF_ACTION_CATEGORY"))
ve <- ve[PWSID %in% ids_d1]
ve[, rule_num := suppressWarnings(as.numeric(RULE_CODE))]
ve_ioc <- ve[rule_num %in% ioc_rules]
cat(sprintf("IOC-restricted violation/enforcement rows (RULE_CODE in {331,332,333}): %d of %d\n",
            nrow(ve_ioc), nrow(ve)))
rm(ve); gc()

# ── 3a. Formal enforcement, PWSID-year, IOC-restricted ──────────────────────
enf <- ve_ioc[, .(PWSID, COMPL_PER_BEGIN_DATE, ENF_ACTION_CATEGORY)]
...
cat(sprintf("PWSID-years with IOC formal enforcement: %d\n", sum(enf_agg$any_formal)))
rm(enf); gc()

# ── 3b. MR violation onsets, PWSID-year, IOC-restricted ─────────────────────
...
onsets <- unique(ve_ioc[, .(PWSID, VIOLATION_ID, NON_COMPL_PER_BEGIN_DATE, VIOLATION_CATEGORY_CODE)])
...
cat(sprintf("PWSID-years with an IOC MR violation onset: %d\n", nrow(mr_agg)))
rm(ve_ioc, onsets, mr_onsets); gc()
...
# ── 6. OLS regressions ────────────────────────────────────────────────────────
fml_nofe     <- any_snsv ~ mr_violation + upstream_mines_above_median + any_formal
fml_yearfe   <- any_snsv ~ mr_violation + upstream_mines_above_median + any_formal | year
fml_pwsidfe  <- any_snsv ~ mr_violation + upstream_mines_above_median + any_formal | PWSID
fml_bothfe   <- any_snsv ~ mr_violation + upstream_mines_above_median + any_formal | PWSID + year
...
cat("\n--- OLS: sanitary visit on IOC MR violation + above-median upstream mines + IOC formal enforcement (D1 main 2SLS panel) ---\n")
etable(m1, m2, m3, m4,
...
out_tex <- file.path(ROOT, "output/reg/sanitary_visit_mr_upstream_mines_ols_ioc.tex")

dict_main <- c(
  "mr_violation"                 = "MR violation (IOC)",
  "upstream_mines_above_median"  = "Upstream mines $>$ median",
  "any_formal"                   = "Formal enforcement (IOC)",
  "PWSID"                        = "CWS"
)

etable(m1, m2, m3, m4,
       title          = "Effect of IOC MR Violations, Upstream Coal Mining, and IOC Formal Enforcement on Sanitary Visits (D1 Downstream Sample)",
       label          = "tab:sanitary_visit_mr_upstream_mines_ols_ioc",
       dict           = dict_main,
       headers        = list("Fixed effects" = list("None" = 1, "Year" = 1, "CWS" = 1, "Year + CWS" = 1)),
       fitstat        = ~ n + r2,
       notes          = paste0(
         ...
         "Dependent variable: sanitary visit = 1 if a site visit with VISIT\\_REASON\\_CODE in ",
         "\\{SNSV, SSVF\\} occurs in that calendar year (",
         sprintf("%.1f", 100 * mean(panel_d1$any_snsv)), "\\% of panel). MR violation (IOC) = 1 if a ",
         "VIOLATION\\_CATEGORY\\_CODE = MR violation restricted to IOC rules — nitrate (331), ",
         "arsenic (332), inorganic chemicals (333) — has its onset (NON\\_COMPL\\_PER\\_BEGIN\\_DATE) ",
         "in that calendar year (", sprintf("%.1f", 100 * mean(panel_d1$mr_violation)),
         "\\% of panel). Upstream mines $>$ median = 1 if num\\_coal\\_mines\\_upstream\\_sum exceeds ",
         "the median (", median_upstream_mines, ") across CWS-years in this D1 main downstream 2SLS ",
         "panel. Formal enforcement (IOC) = 1 if any enforcement action with ENF\\_ACTION\\_CATEGORY ",
         "= Formal, restricted to IOC rules (331, 332, 333), is ongoing in the calendar year (",
         sprintf("%.1f", 100 * mean(panel_d1$any_formal)),
