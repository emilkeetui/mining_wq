# ============================================================
# Script: sanitary_visit_mr_violation_iterative_12mo_test.r
# Purpose: TEST VARIANT of sanitary_visit_mr_violation_iterative.r using
#          ±12-month (instead of ±6-month) lead/lag visit-window indicators.
#          CWS-month panel (strictly-downstream 2SLS sample, 1985-2005)
#          testing whether sanitary visit and enforcement visit timing
#          predicts an MR violation onset. Binary before12/after12 window
...
# Outputs: output/reg/sanitary_visit_mr_violation_iterative_12mo_test.tex
...
rhs_san  <- "san_before12 + san_after12"
rhs_enfv <- "enfv_before12 + enfv_after12"
...
out_tex <- file.path(ROOT, "output/reg/sanitary_visit_mr_violation_iterative_12mo_test.tex")
note_main <- paste0(
  "TEST SPEC: ±12-month lead/lag visit-window terms (instead of ±6-month). ",
  ...
  "in that CWS-month. before12 = 1 if the CWS-month falls in the 12 calendar months ",
  "preceding any visit of that type; after12 = 1 if it falls in the 12 months following ",
...
row_labels_in_order <- gsub("([.()\\\\%$^*+?{}\\[\\]|])", "\\\\\\1",
  dict[c("san_before12", "san_after12", "enfv_before12", "enfv_after12",
...
do.call(etable, c(
  ...
  list(title     = "Sanitary and Enforcement Visit Timing (12-Month Windows) and MR Violation Onset",
       label     = "tab:sanitary_visit_mr_violation_iterative_12mo_test",
       dict      = dict,
       notes     = note_main,
       fitstat   = ~n,
