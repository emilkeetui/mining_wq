# ============================================================
# Script: syr2_test_mr_violation_bychem_iterative.r
# Purpose: CWS-month panel (strictly-downstream 2SLS sample, 1997-2005)
#          testing whether SYR2 test timing predicts an MR violation onset,
#          run separately by chemical-matched pair (mirrors
#          syr2_test_mr_violation_iterative.r, but disaggregated):
#            (a) nitrate MR (rule 331)   ~ nitrate SYR2 test timing
#            (b) arsenic MR (rule 332)   ~ arsenic SYR2 test timing
#            (c) IOC MR (rule 333, inorganic chemicals) ~ barium + selenium
#                SYR2 test timing
#          Binary before6/after6 window indicators built for SYR2 test
#          months of the relevant chemical(s) in each spec, plus two
#          lagged-level controls carried forward (LOCF) from the most
#          recent test at/before that CWS-month (mirrors the z-scoring in
#          sanitary_visit_syr2_enforcement_zscore_iterative.r /
#          monitoring_retesting_hazard.r): a within-CWS-x-chemical z-score
#          of the tested value, and a within-CWS-x-chemical z-score of the
#          running mean value through that test. For the IOC spec (barium +
#          selenium), the two chemicals' lagged z-scores are averaged.
...
# Outputs: output/reg/syr2_test_mr_violation_bychem_iterative.tex
...
# Within-CWS x chemical z-score of the tested value, and the running mean of
# that z-score through each test (mirrors sanitary_visit_syr2_enforcement_
# zscore_iterative.r / monitoring_retesting_hazard.r).
...
syr2_all[, mean_value_z := cumsum(value_z) / seq_len(.N), by = .(PWSID, CHEMID_name)]

# Last test that CWS-month x chemical, used to build the LOCF lagged series below.
syr2_pm_chem <- syr2_all[, .(value_z = value_z[.N], mean_value_z = mean_value_z[.N]),
...
# For a set of chemicals, build the CWS-month lagged (LOCF) value_z and
# mean_value_z series -- the most recent test's z-scores carried forward to
# every subsequent month. When >1 chemical (IOC spec), average across
# chemicals' lagged series for that CWS-month.
build_lagged_zscores <- function(chems) {
  ...
  locf_one_chem <- function(chem) {
    dd <- d[CHEMID_name == chem, .(PWSID, month_idx, value_z, mean_value_z)]
  ...
  stacked[, .(value_z = mean(value_z, na.rm = TRUE),
              mean_value_z = mean(mean_value_z, na.rm = TRUE)),
          by = .(PWSID, month_idx)]
}
...
build_and_run <- function(spec_name, rule, chems) {
  ...
  # Lagged z-score controls stay NA before a CWS's first test of the relevant
  # chemical(s) -- feols() drops those rows for the models that include them.
  ...
  rhs_windows <- "syr2_before6 + syr2_after6"
  rhs_full    <- paste(rhs_windows, "value_z", "mean_value_z", sep = " + ")

  m_no_fe <- run_spec_no_fe("mr_violation", rhs_full, skel)
  m_fe    <- run_spec(      "mr_violation", rhs_full, skel)

  cat(sprintf("\n--- %s MR violation, SYR2 test timing, no FE ---\n", spec_name))
  print(summary(m_no_fe))
  cat(sprintf("\n--- %s MR violation, SYR2 test timing, CWS+month FE ---\n", spec_name))
  print(summary(m_fe))
...
dict <- c(
  ...
  mean_value_z   = "Lagged running mean value (z-score)",
  PWSID          = "CWS",
  calendar_month = "Calendar month (Jan-Dec)"
)

out_tex <- file.path(ROOT, "output/reg/syr2_test_mr_violation_bychem_iterative.tex")
note_main <- paste0(
  "Sample: downstream CWSs, CWS-months 1997-01 to 2005-12. Each pair of columns ",
  "restricts to CWSs with >=1 SYR2 measurement of the relevant chemical(s): ",
  "nitrate (", results$nitrate$n_pwsids, " CWSs, N=", results$nitrate$n,
  "), arsenic (", results$arsenic$n_pwsids, " CWSs, N=", results$arsenic$n,
  "), barium/selenium (", results$ioc$n_pwsids, " CWSs, N=", results$ioc$n,
  "). Outcome = 1 if an MR violation onset under the matched IOC rule occurs in that ",
  "CWS-month: nitrate rule 331 (cols 1-2), arsenic rule 332 (cols 3-4), inorganic ",
  "chemicals rule 333 (cols 5-6). before6 = 1 if the CWS-month falls in the 6 ",
  "calendar months preceding any SYR2 (Six-Year Review 2) test of the matched ",
  "chemical(s) (nitrate for cols 1-2, arsenic for cols 3-4, barium or selenium for ",
  "cols 5-6); after6 = 1 if it falls in the 6 months following one; both are 0 ",
  "outside any such window. Lagged test value (z-score) = (tested value - CWS x ",
  "chemical mean) / (CWS x chemical SD) from the most recent test of the matched ",
  "chemical(s) at or before that CWS-month, carried forward until the next test; ",
  "lagged running mean value (z-score) = running mean of the z-scored values through ",
  "that same lagged test, also carried forward. For the IOC spec (cols 5-6), barium ",
  "and selenium's lagged z-scores are averaged. Both are undefined (and the CWS-month ",
  "dropped from estimation) before a CWS's first test of the matched chemical(s). ",
  "Columns (2), (4), (6) include CWS fixed effects and calendar-month-of-year fixed ",
  "effects (Jan-Dec, not absolute calendar time), which absorb seasonality in violation ",
  "reporting rather than the specific year-month. Columns (1), (3), (5) have no fixed ",
  "effects. SEs clustered at the CWS (PWSID) level in all columns."
)
...
do.call(etable, c(
  ...
  list(title     = "SYR2 Test Timing and MR Violation Onset, by Chemical",
       label     = "tab:syr2_test_mr_violation_bychem_iterative",
       dict      = dict,
