# ============================================================
# Script: sanitary_visit_syr2_enforcement_zscore_iterative.r
...
#          for five chemicals (arsenic, nitrate, selenium, barium, chromium)
#          predicts an enforcement visit. For each chemical: a 6-month
#          lead/lag pair of indicators for whether a test occurred, a
#          within-CWS-x-chemical z-score of the tested value, and a
#          within-CWS-x-chemical z-score of the running mean value through
#          that test (mirrors the z-scoring in monitoring_retesting_hazard.r).
#          Dependent variable: 1 if an enforcement visit (FENF/INVG/EMRG)
#          occurred in that CWS-month (cols 1-3) or 1 if a sanitary visit
#          (SNSV/SNSP/SSVF) occurred in that CWS-month (cols 4-6).
#          Col 1: test-occurrence leads/lags only, no FE (enforcement visit)
#          Col 2: + z-score value/mean controls, no FE (enforcement visit)
#          Col 3: + z-score value/mean controls, CWS + calendar-month FE (enforcement visit)
#          Col 4: test-occurrence leads/lags only, no FE (sanitary visit)
#          Col 5: + z-score value/mean controls, no FE (sanitary visit)
#          Col 6: + z-score value/mean controls, CWS + calendar-month FE (sanitary visit)
...
# Outputs: output/reg/sanitary_visit_syr2_enforcement_zscore_iterative.tex
# Author: EK  Date: 2026-07-12
# ============================================================
...
# Running mean of the z-scored value through this test, per CWS x chemical
setorder(syr2, PWSID, CHEMID_name, sample_dt)
syr2[, mean_value_z := cumsum(value_z) / seq_len(.N), by = .(PWSID, CHEMID_name)]

# One row per CWS-month x chemical: last test that month (value_z, mean_value_z),
# and a test-occurred flag used to build the before6/after6 windows below.
syr2_pm_chem <- syr2[, .(value_z = value_z[.N], mean_value_z = mean_value_z[.N]),
                      by = .(PWSID, CHEMID_name, month_idx)]
test_dates_chem <- unique(syr2[, .(PWSID, CHEMID_name, month_idx)])

rm(syr2); gc()

# ── 2. Per-chemical 6-month lead/lag "test occurred" windows ─────────────────
...
# ── 3. Per-chemical z-score columns, wide (0 in months with no test of that chem) ─
zscore_wide_list <- lapply(TARGET_CHEMS, function(chem) {
  d <- syr2_pm_chem[CHEMID_name == chem, .(PWSID, month_idx, value_z, mean_value_z)]
  setnames(d, c("value_z", "mean_value_z"),
              c(paste0(chem, "_value_z"), paste0(chem, "_mean_value_z")))
  d
})
zscore_wide <- Reduce(function(a, b) merge(a, b, by = c("PWSID", "month_idx"), all = TRUE),
                       zscore_wide_list)
...
skel <- merge(skel, test_win_pm,     by = c("PWSID", "month_idx"), all.x = TRUE)
skel <- merge(skel, zscore_wide,     by = c("PWSID", "month_idx"), all.x = TRUE)
...
win_cols <- as.vector(t(outer(TARGET_CHEMS, c("_before6", "_after6"), paste0)))
z_cols   <- as.vector(t(outer(TARGET_CHEMS, c("_value_z", "_mean_value_z"), paste0)))
fill0_cols <- c("enforcement_visit", "sanitary_visit", win_cols, z_cols)
...
# ── 6. Regressions ─────────────────────────────────────────────────────────────
rhs_windows <- paste(win_cols, collapse = " + ")
rhs_zscores <- paste(z_cols, collapse = " + ")
...
cat("\n--- Enforcement visit, test windows only, no FE ---\n");           print(summary(m1))
cat("\n--- Enforcement visit, test windows + z-scores, no FE ---\n");     print(summary(m2))
cat("\n--- Enforcement visit, test windows + z-scores, CWS+month FE ---\n"); print(summary(m3))
cat("\n--- Sanitary visit, test windows only, no FE ---\n");              print(summary(m4))
cat("\n--- Sanitary visit, test windows + z-scores, no FE ---\n");        print(summary(m5))
cat("\n--- Sanitary visit, test windows + z-scores, CWS+month FE ---\n"); print(summary(m6))
...
chem_labels <- c(arsenic = "Arsenic", nitrate = "Nitrate", selenium = "Selenium",
                  barium = "Barium", chromium = "Chromium")

dict <- c(
  enforcement_visit = "Visit occurred in month t",
  sanitary_visit     = "Visit occurred in month t",
  setNames(paste(chem_labels[TARGET_CHEMS], "test lead (1, 6) months"),
            paste0(TARGET_CHEMS, "_before6")),
  setNames(paste(chem_labels[TARGET_CHEMS], "test lag (-6, -1) months"),
            paste0(TARGET_CHEMS, "_after6")),
  setNames(paste(chem_labels[TARGET_CHEMS], "value (z-score)"),
            paste0(TARGET_CHEMS, "_value_z")),
  setNames(paste(chem_labels[TARGET_CHEMS], "mean value (z-score)"),
            paste0(TARGET_CHEMS, "_mean_value_z")),
  PWSID          = "CWS",
  calendar_month = "Calendar month (Jan-Dec)"
)

out_tex <- file.path(ROOT, "output/reg/sanitary_visit_syr2_enforcement_zscore_iterative.tex")
note_main <- paste0(
  ...
  "(SNSV/SNSP/SSVF) occurred at that CWS in that calendar month. For each chemical, ",
  "before6 = 1 if the CWS-month falls in the 6 calendar months preceding a SYR2 test of ",
  "that chemical; after6 = 1 if it falls in the 6 months following one; both are 0 ",
  "outside any such window. Value (z-score) = (tested value - CWS x chemical mean) / ",
  "(CWS x chemical SD), taken from the most recent test of that chemical that CWS-month; ",
  "mean value (z-score) = running mean of the z-scored values through that test. Both ",
  "z-scores are 0 in CWS-months with no test of that chemical (and 0 in months with no ",
  "prior test for the running mean). Columns (3) and (6) include CWS fixed effects and ",
  "calendar-month-of-year fixed effects (Jan-Dec, not absolute calendar time), which ",
  "absorb seasonality in testing, enforcement, and sanitary-visit activity rather than ",
  "the specific year-month. Columns (1), (2), (4), (5) have no fixed effects. N=",
  nrow(skel), " in all columns. SEs clustered at the CWS (PWSID) level in all columns."
)

do.call(etable, c(
  list(m1, m2, m3, m4, m5, m6),
  list(title     = "SYR2 Testing Timing, Contaminant Levels, and Enforcement/Sanitary Visits",
       label     = "tab:sanitary_visit_syr2_enforcement_zscore_iterative",
       dict      = dict,
