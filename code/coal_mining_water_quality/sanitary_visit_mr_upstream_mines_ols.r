# ============================================================
# Script: sanitary_visit_mr_upstream_mines_ols.r
...
#          MR violation here includes ALL MR violations in the D1 sample
#          (not restricted to IOC contaminants) — see
#          sanitary_visit_mr_upstream_mines_ols_ioc.r for the IOC-restricted
#          version (nitrate/arsenic/inorganic chemicals only).
...
# Outputs: output/reg/sanitary_visit_mr_upstream_mines_ols.tex
...
# ── 3b. MR violation onsets, PWSID-year (ALL MR violations) ─────────────────
...
cat("\n--- OLS: sanitary visit on MR violation + above-median upstream mines + formal enforcement (D1 main 2SLS panel) ---\n")
etable(m1, m2, m3, m4,
...
out_tex <- file.path(ROOT, "output/reg/sanitary_visit_mr_upstream_mines_ols.tex")
...
etable(m1, m2, m3, m4,
       title          = "Effect of MR Violations, Upstream Coal Mining, and Formal Enforcement on Sanitary Visits (D1 Downstream Sample)",
       label          = "tab:sanitary_visit_mr_upstream_mines_ols",
       dict           = dict_main,
       headers        = list("Fixed effects" = list("None" = 1, "Year" = 1, "CWS" = 1, "Year + CWS" = 1)),
       fitstat        = ~ n + r2,
       notes          = paste0(
         ...
         sprintf("%.1f", 100 * mean(panel_d1$any_snsv)), "\\% of panel). MR violation = 1 if a ",
         "VIOLATION\\_CATEGORY\\_CODE = MR violation (any contaminant) has its onset ",
         "(NON\\_COMPL\\_PER\\_BEGIN\\_DATE) in that calendar year (",
         sprintf("%.1f", 100 * mean(panel_d1$mr_violation)), "\\% of panel). Upstream mines $>$ median = 1 ",
         "if num\\_coal\\_mines\\_upstream\\_sum exceeds the median (", median_upstream_mines,
         ") across CWS-years in this D1 main downstream 2SLS panel. Formal enforcement = 1 if any ",
         "enforcement action with ENF\\_ACTION\\_CATEGORY = Formal is ongoing in the calendar year (",
