...
# upstream hucs flow into mine hucs and do not have a mine upstream or in the same huc
# 474 such hucs
...
# concat the huc list of upstream, mining, and downstream of mining hucs
downstreamhuc = huc[huc['huc12'].isin(minehuc.tohuc)]
downstreamhuc['minehuc'] = 'downstream_of_mine'
downstreamhuc[['minehuc', 'huc12', 'tohuc']]
huclist = pd.concat([downstreamhuc, huclist], ignore_index=True)
...
# remove pwsid's that get their water from a source that isnt upstream both an upstream and a downstream huc
...
# Optional: recompute and inspect what's left
minehuc_types = facility.groupby('PWSID')['minehuc'].unique()

# For display consistency (optional): normalize NaNs to string 'nan'
...
onelvldwn = (
    ...
    .agg(
        btu=('btu', 'mean'),
        sulfur=('sulfur', 'mean'),
        production_short_tons_coal=('production_short_tons_coal', 'mean'),
        num_coal_mines=('num_coal_mines', 'max')
    )
)
...
enf_keep = violation_keep

# enforcement action category

# enforcement action origin

# violation cat X rule family X enforcement action
enf_keep["violation_cat_rule_fam_enf_action"] = (
    enf_keep["violation_cat_rule_fam"].astype(str) + 
    "_" + 
    enf_keep["ENFORCEMENT_ACTION_TYPE_CODE"].astype(str)
)

# violation cat X rule family X enforcement origin
enf_keep["violation_cat_rule_fam_enf_origin"] = (
    enf_keep["violation_cat_rule_fam"].astype(str) + 
    "_" + 
    enf_keep["ENF_ORIGINATOR_CODE"].astype(str)
)

enf_keep = pd.get_dummies(enf_keep,
                          columns=['violation_cat_rule_fam_enf_origin', 
                                   'violation_cat_rule_fam_enf_action',
                                   'ENF_ACTION_CATEGORY',
                                   'ENF_ORIGINATOR_CODE'],
                          dummy_na=True,
                          dtype=int)
...
# Get all column names that start with 'a' or 'b'
columns_groupby = [col for col in enf_keep.columns if col.startswith(('violation_cat_rule_fam_enf_origin',
                                                                      'violation_cat_rule_fam_enf_action',
                                                                      'ENF_ACTION_CATEGORY',
                                                                      'ENF_ORIGINATOR_CODE'))]
enf_keep = enf_keep.groupby(['PWSID','year'])[columns_groupby].max().reset_index()
...
# hot encode violation_cat_rule_fam
violation_keep_expanded = pd.get_dummies(violation_keep_expanded,
                          columns=["rule_family",
                                   "RULE_CODE",
                                   "RULE_GROUP_CODE",
                                   "VIOLATION_CATEGORY_CODE",
                                   "violation_cat_rule_fam",
                                   "IS_HEALTH_BASED_IND",
                                   "IS_MAJOR_VIOL_IND"],
                          dummy_na=True,
                          dtype=int)
columns_groupby = [col for col in violation_keep_expanded.columns if col.startswith(('violation_cat_rule',
                                                                                     'rule_family',
                                                                                     'RULE_CODE',
                                                                                     'RULE_GROUP_CODE',
                                                                                     'VIOLATION_CATEGORY_CODE',
...
violation_keep = (violation_keep_expanded.groupby(['PWSID', 'year'])
                  .agg(**{col: ('{}'.format(col), 'max') for col in columns_groupby + ['multi_year_viol']},
                       num_violations=('PWSID', 'size'))
                  .reset_index())

print(violation_keep)
...
print(enf_keep)
water_sys = water_sys.merge(enf_keep, how='left', on = ['PWSID', 'year'])
print(water_sys.columns)
