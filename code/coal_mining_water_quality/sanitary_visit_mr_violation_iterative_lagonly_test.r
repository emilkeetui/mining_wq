# ============================================================
# Script: sanitary_visit_mr_violation_iterative_lagonly_test.r
# Purpose: TEST VARIANT of sanitary_visit_mr_violation_iterative.r keeping
#          only the lag (after12) visit-window terms -- the lead (before12)
#          terms are dropped from the regression RHS entirely (still built
#          in the data step, just excluded from the model formulas/table).
#          CWS-month panel (strictly-downstream 2SLS sample, 1985-2005)
#          testing whether sanitary visit and enforcement visit timing
#          predicts an MR violation onset.
#          Col 1: sanitary visit lag only, no FE (full panel)
#          Col 2: sanitary visit lag only, CWS + calendar-month FE (full panel)
#          Col 3: sanitary visit lag only + n_prior_violations +
#                 pct_mcl_last_max, CWS + calendar-month FE (SYR2-restricted)
#          Col 4: enforcement visit lag only, no FE (full panel)
#          Col 5: enforcement visit lag only, CWS + calendar-month FE (full panel)
#          Col 6: enforcement visit lag only + n_prior_violations +
#                 pct_mcl_last_max, CWS + calendar-month FE (SYR2-restricted)
...
# Outputs: output/reg/sanitary_visit_mr_violation_iterative_lagonly_test.tex
...
# ── 2. Visit groups ───────────────────────────────────────────────────────────
# Two groups, each producing a before12/after12 window indicator.
visit_groups <- list(
...
# For each group: deduplicate visit months, expand into ±12 month windows.
build_visit_windows <- function(codes, prefix) {
  ...
  before_long <- visits_dd[, .(month_idx = seq(month_idx - 12L, month_idx - 1L)),
                              by = .(PWSID, visit_month = month_idx)]
  after_long  <- visits_dd[, .(month_idx = seq(month_idx + 1L, month_idx + 12L)),
                              by = .(PWSID, visit_month = month_idx)]
  ...
  b_pm[[paste0(prefix, "_before12")]] <- 1L
  a_pm[[paste0(prefix, "_after12")]]  <- 1L

  m <- merge(b_pm, a_pm, by = c("PWSID", "month_idx"), all = TRUE)
  cat(sprintf("    before12=1: %d  after12=1: %d\n",
              sum(m[[paste0(prefix, "_before12")]], na.rm = TRUE),
              sum(m[[paste0(prefix, "_after12")]],  na.rm = TRUE)))
  m
}
...
visit_win_cols <- c("san_before12", "san_after12",
                     "enfv_before12", "enfv_after12")
...
dict <- c(
  ...
  san_before12       = "Sanitary visit lead (1, 12) months",
  san_after12        = "Sanitary visit lag (-12, -1) months",
  enfv_before12      = "Enforcement visit lead (1, 12) months",
  enfv_after12       = "Enforcement visit lag (-12, -1) months",
...
out_tex <- file.path(ROOT, "output/reg/sanitary_visit_mr_violation_iterative_lagonly_test.tex")
note_main <- paste0(
  "TEST SPEC: lag-only (after12) visit-window terms; lead (before12) terms are dropped ",
  "from the RHS. Sample: strictly downstream CWSs (", length(sample_pwsids), "), ",
  "CWS-months 1985-01 to 2005-12 in columns (1), (2), (4), (5). Outcome = 1 if an MR ",
  "violation onset occurs in that CWS-month. after12 = 1 if the CWS-month falls in the ",
  "12 months following any visit of that type; 0 otherwise. Sanitary visits = ",
  "SNSV/SNSP/SSVF; enforcement visits = FENF/INVG/EMRG. N\\_prior\\_violations = ",
  "cumulative count of prior violation onsets (any category) at that CWS. Last max ",
  "conc. (\\% of MCL) = running-max SYR2 contaminant concentration as a share of the ",
  "MCL. Columns (2), (3), (5), (6) include CWS and year-month fixed effects; columns ",
  "(1) and (4) have no fixed effects. Columns (3) and (6) add the prior-violation and ",
  "concentration controls and are restricted to CWSs with >=1 SYR2 measurement and ",
  "CWS-months within the SYR2 window (1998-2005), N=", nrow(spec3_dt), ". Columns (1), ",
  "(2), (4), (5) use the full panel, N=", nrow(skel), ". SEs clustered at the CWS ",
...
row_labels_in_order <- gsub("([.()\\\\%$^*+?{}\\[\\]|])", "\\\\\\1",
  dict[c("san_after12", "enfv_after12",
...
do.call(etable, c(
  ...
  list(title     = "Sanitary and Enforcement Visit Timing (Lag Only) and MR Violation Onset",
       label     = "tab:sanitary_visit_mr_violation_iterative_lagonly_test",
       dict      = dict,
       notes     = note_main,
       fitstat   = ~n,
