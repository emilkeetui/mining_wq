...
#          MR violation onset AND a same-year sanitary visit AND SYR2
#          readings for arsenic, chromium, barium, nitrate, and selenium
#          below their respective sample means (whichever of the 5 are
#          tested that CWS-year; a CWS-year qualifies if every chemical it
#          was tested for that year came in below that chemical's mean).
...
# Outputs: output/sum/sanitary_visit_formal_enforcement_rate_syr2_below_mean.tex
...
# ── 4c. Row 6: row-3 sample + SYR2 readings below chemical means ────────────
...
# require each one to be below its fixed published mean (SYR2_MEANS). A
# CWS-year qualifies based on whichever of the 5 chemicals it was actually
# tested for that year (not all 5 required); CWS-years with zero SYR2
# readings for any of the 5 chemicals that year do not qualify.
...
syr2[, below_mean := VALUE < chem_mean]

# CWS-year qualifies iff every tested chemical that year is below its mean
# (all() over the readings present; a CWS-year with 0 rows for these
# chemicals is absent from below_mean_py and therefore does not qualify).
below_mean_py <- syr2[, .(all_below_mean = as.integer(all(below_mean))), by = .(PWSID, year)]
below_mean_py <- below_mean_py[all_below_mean == 1, .(PWSID, year)]
cat("CWS-years where all tested SYR2 chemicals (of the 5) are below their mean:",
    nrow(below_mean_py), "\n")

row6_dt <- row3_dt[below_mean_py, on = .(PWSID, year), nomatch = NULL]
cat("CWS-years with MR violation + same-year sanitary visit + SYR2 below mean:",
    nrow(row6_dt), "\n")
...
rows6 <- data.table(
  sample = c("All CWS-years (downstream 2SLS panel)",
             ...
             "CWS-years with an MR violation, sanitary visit (same year), and SYR2 below mean"),
...
cat("\n--- Formal enforcement rate table (6 rows, test version) ---\n")
print(rows6)
...
notes_6row <- paste0(
  ...
  "violation and same-year sanitary visit) to CWS-years where every SYR2 (6-Year Review) ",
  "reading of arsenic, nitrate, barium, chromium, or selenium taken that year is below that ",
  "chemical's mean, using the fixed published means from Table~",
  "\\ref{tab:6yr_huc02fe_inorg_val_sumstats_ravalli_2005} (", mean_str, "); a CWS-year ",
  "qualifies based on whichever of the 5 chemicals it was actually tested for that year (not ",
  "all 5 required), and CWS-years with no SYR2 reading of any of the 5 chemicals that year do ",
...
out_tex <- file.path(ROOT, "output/sum/sanitary_visit_formal_enforcement_rate_syr2_below_mean.tex")
lines_tex <- c(
  "\\begin{table}[htbp]",
  paste0("\\caption{\\label{tab:sanitary_visit_formal_enforcement_rate_syr2_below_mean} Formal ",
         "Enforcement Rate, by Sample Restriction, Visit Timing, and SYR2 Level (Downstream ",
         "CWSs, ", YR_LO, "--", YR_HI, ") -- Test Version}"),
  "\\bigskip",
  "\\centering",
