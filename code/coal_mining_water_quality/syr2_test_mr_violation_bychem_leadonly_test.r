# ============================================================
# Script: syr2_test_mr_violation_bychem_leadonly_test.r
# Purpose: Test variant of syr2_test_mr_violation_bychem_iterative.r --
#          same CWS-month panel (strictly-downstream 2SLS sample,
#          1997-2005) and same chemical-matched pair specs (nitrate/331,
#          arsenic/332, IOC-barium+selenium/333), but the regression RHS
...
#          syr2_after6 (the lag/after-window indicator) is dropped.
...
# Outputs: output/reg/syr2_test_mr_violation_bychem_leadonly_test.tex
...
build_and_run <- function(spec_name, rule, chems) {
  ...
  # Lead-only RHS: drop syr2_after6, keep syr2_before6 + value_z + mean_value_z.
  rhs_full <- "syr2_before6 + value_z + mean_value_z"

  m_no_fe <- run_spec_no_fe("mr_violation", rhs_full, skel)
  m_fe    <- run_spec(      "mr_violation", rhs_full, skel)

  cat(sprintf("\n--- %s MR violation, SYR2 test lead only, no FE ---\n", spec_name))
  print(summary(m_no_fe))
  cat(sprintf("\n--- %s MR violation, SYR2 test lead only, CWS+month FE ---\n", spec_name))
  print(summary(m_fe))
...
out_tex <- file.path(ROOT, "output/reg/syr2_test_mr_violation_bychem_leadonly_test.tex")
note_main <- paste0(
  "TEST SPEC: lead-only variant of syr2_test_mr_violation_bychem_iterative.r -- ",
  "the SYR2 test lag (-6,-1) months indicator is dropped from the RHS; only the ",
  "lead window, lagged test value (z-score), and lagged running mean value ",
  "(z-score) are included. Sample: downstream CWSs, CWS-months 1997-01 to ",
  "2005-12. Each pair of columns restricts to CWSs with >=1 SYR2 measurement of ",
  ...
  "forward until the next test; lagged running mean value (z-score) = running ",
  "mean of the z-scored values through that same lagged test, also carried ",
  "forward. For the IOC spec (cols 5-6), barium and selenium's lagged z-scores ",
  "are averaged. Both are undefined (and the CWS-month dropped from estimation) ",
  "before a CWS's first test of the matched chemical(s). Columns (2), (4), (6) ",
  "include CWS fixed effects and calendar-month-of-year fixed effects (Jan-Dec, ",
  "not absolute calendar time), which absorb seasonality in violation reporting ",
  "rather than the specific year-month. Columns (1), (3), (5) have no fixed ",
...
do.call(etable, c(
  ...
  list(title     = "SYR2 Test Lead and MR Violation Onset, by Chemical (Test: Lead-Only)",
       label     = "tab:syr2_test_mr_violation_bychem_leadonly_test",
       dict      = dict,
