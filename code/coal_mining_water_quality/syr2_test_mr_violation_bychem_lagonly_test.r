# ============================================================
# Script: syr2_test_mr_violation_bychem_lagonly_test.r
...
#          but the regression RHS contains ONLY lagged (past) variables,
#          no leads and no lagged running mean:
#            - syr2_after6 (SYR2 test lag (1,6) months -- test occurred in
#              the past 6 months)
#            - value_z (lagged test value, EXPANDING-WINDOW z-score, from
#              the most recent past test, LOCF-carried forward). Unlike the
#              source script's full-sample z-score, each test is standardized
#              using only the CWS x chemical mean/SD of tests up to and
#              including that one (Welford's online algorithm), so value_z
#              is forecast-valid -- it never uses information from tests that
#              haven't happened yet as of that sample date. A CWS x
#              chemical's first test has an undefined z-score (SD needs >=2
#              points) and is dropped.
#          Tests whether an MR violation is more/less likely in the 6
#          months after an SYR2 test, and after a higher (vs. lower)
#          z-scored test value.
...
# Outputs: output/reg/syr2_test_mr_violation_bychem_lagonly_test.tex
...
# Within-CWS x chemical EXPANDING-WINDOW z-score of the tested value: each
# test is standardized using only the mean/SD of tests up to and including
# that one (not the full-sample mean/SD), so value_z is forecast-valid --
# it never uses information from tests that haven't happened yet as of that
# sample date. Computed via Welford's online algorithm (running mean + running
# sum of squared deviations from the running mean, updated one test at a time).
# A CWS x chemical's first test has an undefined z-score (SD needs >=2 points).
setorder(syr2_all, PWSID, CHEMID_name, sample_dt)

welford_expanding_z <- function(x) {
  n_obs <- length(x)
  z     <- rep(NA_real_, n_obs)
  mean_i <- 0
  m2_i   <- 0
  for (i in seq_len(n_obs)) {
    delta  <- x[i] - mean_i
    mean_i <- mean_i + delta / i
    m2_i   <- m2_i + delta * (x[i] - mean_i)
    if (i >= 2L) {
      sd_i <- sqrt(m2_i / (i - 1L))
      z[i] <- (x[i] - mean_i) / sd_i
    }
...
syr2_all[, value_z := welford_expanding_z(VALUE), by = .(PWSID, CHEMID_name)]
...
# before6 is still computed (kept in the panel) but is not included in the
# regression RHS below -- only syr2_after6 is used from this pair.
...
build_and_run <- function(spec_name, rule, chems) {
  ...
  # Lag-only RHS: syr2_after6 + value_z only.
  rhs_full <- "syr2_after6 + value_z"

  m_no_fe <- run_spec_no_fe("mr_violation", rhs_full, skel)
  m_fe    <- run_spec(      "mr_violation", rhs_full, skel)

  cat(sprintf("\n--- %s MR violation, SYR2 test lag only, no FE ---\n", spec_name))
  print(summary(m_no_fe))
  cat(sprintf("\n--- %s MR violation, SYR2 test lag only, CWS+month FE ---\n", spec_name))
  print(summary(m_fe))
...
dict <- c(
  ...
  syr2_after6    = "SYR2 test lag (1, 6) months",
  value_z        = "Lagged test value (z-score)",
  PWSID          = "CWS",
  calendar_month = "Calendar month (Jan-Dec)"
)

out_tex <- file.path(ROOT, "output/reg/syr2_test_mr_violation_bychem_lagonly_test.tex")
note_main <- paste0(
  "TEST SPEC: lag-only variant of syr2_test_mr_violation_bychem_iterative.r -- ",
  "the SYR2 test lead (1,6) months indicator and the lagged running mean value ",
  "(z-score) are both dropped from the RHS; only the lag window (test occurred ",
  "in the past 6 months) and the lagged test value (z-score) are included. ",
  "Sample: downstream CWSs, CWS-months 1997-01 ",
  ...
  "(cols 3-4), inorganic chemicals rule 333 (cols 5-6). after6 = 1 if the ",
  "CWS-month falls in the 6 calendar months following any SYR2 (Six-Year Review 2) ",
  ...
  "(tested value - CWS x chemical expanding-window mean) / (CWS x chemical ",
  "expanding-window SD), where the expanding-window mean/SD through a given test ",
  "use only that test and all prior tests of the matched chemical for that CWS ",
  "(Welford's online algorithm), so the z-score never uses information from ",
  "future tests; carried forward (LOCF) from the most recent test at or before ",
  "that CWS-month until the next test. For the IOC spec (cols 5-6), barium and ",
  "selenium's lagged z-scores are averaged. Value is undefined (and the ",
  "CWS-month dropped from estimation) before a CWS's second test of the matched ",
  "chemical(s) (the expanding-window SD requires >=2 prior tests). Columns (2), ",
  "(4), (6) include CWS fixed effects and ",
...
do.call(etable, c(
  ...
  list(title     = "SYR2 Test Lag and MR Violation Onset, by Chemical (Test: Lag-Only)",
       label     = "tab:syr2_test_mr_violation_bychem_lagonly_test",
       dict      = dict,
