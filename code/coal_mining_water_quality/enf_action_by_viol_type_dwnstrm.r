# ============================================================
# Script: enf_action_by_viol_type_dwnstrm.r
...
#          (mining vs non-mining), 1985-2005 — restricted to CWSs in the
#          downstream 2SLS sample (minehuc_downstream_of_mine==1 & minehuc_mine==0)
...
# Outputs: output/sum/enf_action_by_viol_type_dwnstrm.tex
...
# ── 0. Sample PWSID list — CWSs in the downstream 2SLS sample ────────────────
...
pws_ids <- unique(pws_ds$PWSID[downstream_mask])
downstream_states <- sort(unique(pws_ds$STATE_CODE[downstream_mask]))
cat("States in downstream sample:", paste(downstream_states, collapse = ", "), "\n")
cat("CWSs in downstream 2SLS sample:", length(pws_ids), "\n")
...
header_comment <- paste0(
  ...
  "% Sample:  CWSs in the downstream 2SLS sample (minehuc_downstream_of_mine==1 & minehuc_mine==0)\n",
...
state_list <- paste(downstream_states[downstream_states != "0"], collapse = ", ")

lines <- c(
  "\\begin{table}[htbp]",
  "\\centering",
  "\\caption{Enforcement Action Types by Violation Category and Contaminant Group, 1985--2005 (Downstream 2SLS Sample)}",
  "\\label{tab:enf_action_type_dwnstrm}",
  "\\small",
...
for (sec in sections) {
  ...
  for (i in seq_len(nrow(sec$rows))) {
    r   <- sec$rows[i]
    cat_label <- if (i == 1) r$ENF_ACTION_CATEGORY else ""
...
notes <- paste0(
  ...
  "Sample: CWSs in the downstream 2SLS sample (HUC12 at most two steps downstream of a coal mine, ",
  "not colocated), 1985--2005 ",
...
# ── 9. Write output ───────────────────────────────────────────────────────────
out_path <- "Z:/ek559/mining_wq/output/sum/enf_action_by_viol_type_dwnstrm.tex"
