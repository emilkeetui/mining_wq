# ============================================================
# Script: mr_concentration_lag_national.r
# Purpose: National-sample (all PWSIDs, no downstream-of-mine
#          restriction) power fix for the mr_concentration_lag
#          ratchet-avoidance test. The downstream-only mining sample
#          has only 3 SYR2 nitrate readings crossing the 50% MCL
#          quarterly-monitoring trigger (40 CFR 141.23(d)(2)); this
#          script re-estimates the identical specification on the
#          full national SYR2 nitrate sample built by
#          build_mr_concentration_lag_national.py, where trigger-
#          crossings are far more numerous. Ratchet avoidance is a
#          general SDWA mechanism, not specific to mining HUCs, so
#          the national sample is a valid test of the same mechanism.
# Inputs:  clean_data/mr_concentration_lag_national_nitrate.parquet
# Outputs: output/reg/mr_concentration_lag_national.tex
...
# ── Step 4: table helpers (copied verbatim from mr_concentration_lag.r) ───────
...
note_main <- paste0(
  "National SYR2 sample (all PWSIDs, 1998--2005), nitrate only. Outcome: nitrate MR ",
  "(monitoring/reporting) violation in the forward window (1--365 days for the 1-yr column; ",
  "1--182 days for the 6-mon column) following the sample date; placebo columns use the same ",
  "windows measured BEFORE the sample date. near\\_mcl = reading at 50--100\\% of the MCL, ",
  "the 40 CFR 141.23(d)(2) quarterly-monitoring trigger. ",
...
out_tex <- file.path(ROOT, "output/reg/mr_concentration_lag_national.tex")
...
wrap_table_float(out_tex,
  "Nitrate MR violations following a reading above 50\\% of the MCL (national SYR2 sample)",
  label = "tab:mr_concentration_lag_national")
