...
# load the complete huc file
huclist = pd.read_csv("Z:/ek559/mining_wq/clean_data/huc_coal_charac_geom_match.csv", dtype={'huc12': str}, low_memory=False)
...
huclist = pd.read_csv("Z:/ek559/mining_wq/clean_data/huc_coal_charac_geom_match.csv", dtype={'huc12': str})
huclist.drop('Unnamed: 0', axis=1, inplace=True)
df_years = pd.DataFrame({'year': list(range(1983, 2025))})
hucsulfur = huclist.merge(df_years, how='cross')
...
# downstream hucs get the upstream coal production characteristics
downstreamcoal = fin
...
fin = pd.concat([fin, downstreamcoal], axis=0)
             
fin = fin[['huc12','minehuc','year','btu_colocated', 'sulfur_colocated', 'btu_upstream', 'sulfur_upstream', 'production_short_tons_coal', 'num_coal_mines']]
