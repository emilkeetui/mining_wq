...
\begin{itemize}\small
    ...
    \item 85\% of US households drink water from a utility that must treat and self-report its water quality.
\end{itemize}
\end{column}
...
\begin{itemize}
    \item Personal income taxes, Healthcare HIPAA violation disclosure, energy firms'self-disclosure to the Federal Energy Regulatory Commission, public companies' SOX (Sarbanes‑Oxley Act) self-reports, Toxic Release Inventories, and drinking water utilities.
    \item The regulator learns of an exceedance if the firm self-reports.
    \item Water quality violations:
    \begin{itemize}
        \item \textbf{MCL} --- reported contamination above the limit. Formally enforced.
        \item \textbf{MR} --- under-reporting. Saves self-reporting costs and conceals MCL.
    \end{itemize}
    \item Under-reporting incentives increase with contamination:
    \begin{itemize}
        \item testing requirements, and so \textbf{reporting costs}, rise with contamination;
        \item the need to \textbf{conceal} an exceedance rises.
    \end{itemize}
...
\begin{itemize}
    ...
    \end{itemize}
    \item Instrument coal production: upstream coal sulfur \% $\times$ post-1995, the Acid Rain Program Phase I deadline.
    \item Contributions:
    \begin{itemize}
        \item First causal estimates of effect of coal mining on \textbf{treated} drinking water at US utilities.
        \item First evidence that \textbf{the cost of self-reporting}, not only concealment, causes under-reporting (cf.\ \cite{bennear2009sampling}; \cite{zou2021unwatched}).
...
\begin{itemize}\small
    ...
    \item $\uparrow$ mining $\rightarrow$ \textbf{no change} in MCL violations.
    \item $\uparrow$ mining $\rightarrow$ $\uparrow$ \textbf{enforcement visits} ($+$3.8 pp) and \textbf{sanitary visits} ($+$14 pp) but $\downarrow$ \textbf{formal enforcement} ($-$5.7 pp).
...
% ---------------------------------------------------------------- 8
\begin{frame}{The Safe Drinking Water Act (SDWA) monitors through self-reports}
\begin{itemize}
    \item Drinking water utilities with $\geq$ 25 people, $\geq$ 15 connections, year-round.
    \item The utility tests its own water on a schedule and files results with the state.
    \begin{itemize}
        \item Testing needs technical staff, administrative tracking, and laboratories.
...
\begin{itemize}\small
    ...
    \item Sample: Utilities with an intake \textbf{strictly one HUC12 downstream} of a mine watershed. 6,225 utility-years, 333 utilities.
\end{itemize}
\end{frame}
...
\begin{itemize}\small
    ...
    \item Exposure = mines and tons coal in the HUC12s \textbf{directly upstream} of the utility's intakes.
...
\takeaway{Nitrate MR 6.1\%, arsenic 4.0\%, IOC 4.6\% of utility-years. MCL violations: 4, 2, and 20 events across twenty years.}
\end{frame}
...
\end{equation*}
\begin{itemize}\small
    \item $\textit{Concen}_{cht}$: mean annual concentration at utility $c$ in basin $h$, year $t$ (SYR2, 1998--2005).
    \item $\beta$: effect of 10 million additional cumulative short tons mined upstream since 1985.
    \item $\rho_c$: utility fixed effects. $HUC02_h\cdot\tau_t$: basin-specific year effects, absorbing region-wide industrial trends.
...
\presreg[width=1.3\textwidth]{fs_dwnstrm_minevio_ivsum}
...
% ---------------------------------------------------------------- 25
\begin{frame}{And no record of exceedances}
