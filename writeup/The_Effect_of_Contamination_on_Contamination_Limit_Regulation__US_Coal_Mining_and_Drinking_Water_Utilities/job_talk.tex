...
\begin{itemize}
    ...
    \end{itemize}
    \item Firms under-report exactly when reporting is most valuable to the regulator.
\end{itemize}
\end{frame}
...
\begin{itemize}
    ...
    \item Assume $ps < r$ to give utility a reason to under-report.
\end{itemize}
\end{frame}
...
\begin{equation*}
\textit{Concen}_{ct}= \beta\cdot\!\!\sum_{\tau=1985}^{t}\!\!\text{UpstreamCoalProd}_{c\tau} + \rho_c + State_c\cdot\tau_t + \epsilon_{ct}
\end{equation*}
\begin{itemize}\small
    ...
    \item $\rho_c$: utility fixed effects. $State_c\cdot\tau_t$: state-specific year effects, absorbing region-wide industrial trends.
...
\begin{itemize}
    ...
    \item Assumption: utility with large upstream production changes would have trended like those with small ones.[LINK ROBUSTNESS CHECK]
\end{itemize}
\end{frame}
...
\begin{itemize}\small
    ...
    \item \textbf{Exclusion.} Main exclusion test is to look at effect of mining downstream of utility on utility outcomes. [LINK ROBUSTNESS TEST]
