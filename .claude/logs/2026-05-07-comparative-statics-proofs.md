...
## Proposition 4 — Second-Best Penalty Schedule (K&S Implementability)

**Statement.** Under the K&S (2021) second-best optimal enforcement regime, the
expected penalty schedule satisfies:

```
e*'(a) = θ(a)b'(a) > 0
```

and is convex in negligence level a when the type cutoff θ(a) rises sufficiently
fast relative to the concavity of b:

```
e*''(a) = θ'(a)b'(a) + θ(a)b''(a) > 0   iff   θ'(a)b'(a) > θ(a)|b''(a)|
```

K&S (2021, Fig. 2, p. 2977) empirically confirm the estimated schedule is strictly
convex in violation count k. This is the second-best benchmark — not first-best,
which would require the regulator to observe each CWS's type θ directly.

**K&S implementability condition.** By the lemma in Mookherjee and Png (1994), a
negligence schedule a(·) is implemented by a penalty schedule e(·) if and only if
a(·) is nondecreasing. The requisite expected penalty schedule implementing a(θ) is
(K&S eq. 7):

```
e*(a) = θ(a)b(a) − ∫₀^{θ(a)} b[a(v)] dv
```

where θ(a) denotes the highest type τ selecting a(τ) ≤ a — the inverse of the
optimal negligence schedule a*(θ).

**Derivation of e*'(a).** Differentiating e*(a):

```
e*'(a) = θ'(a)b(a) + θ(a)b'(a) − b[a(θ(a))] · θ'(a)
        = θ'(a)b(a) + θ(a)b'(a) − b(a) · θ'(a)   [since a(θ(a)) = a by definition]
        = θ(a)b'(a)
```

Since θ(a) > 0 and b'(a) > 0 by assumption, e*'(a) > 0: the optimal schedule is
strictly increasing in negligence. This is precisely the CWS's FOC — marginal
expected penalty equals compliance cost benefit scaled by the type cutoff. □

**Derivation of e*''(a).** Differentiating e*'(a) = θ(a)b'(a):

```
e*''(a) = θ'(a)b'(a) + θ(a)b''(a)
```

The first term is positive: θ'(a) = 1/a'(θ) > 0 since a*(θ) is strictly increasing
(Proposition 1), and b'(a) > 0 by assumption. The second term is non-positive:
b''(a) ≤ 0 by K&S Assumption 2 (b strictly concave). Convexity of e*(a) holds when:

```
θ'(a)b'(a) > θ(a)|b''(a)|
```

i.e., the rate at which the type cutoff rises dominates the concavity of the
compliance benefit function. If b is linear (b'' = 0), the condition holds strictly
and e*(a) is strictly convex. K&S (2021, Fig. 2) confirm the estimated schedule is
strictly convex in violation count k across its support, consistent with this
condition holding in the data.

**Scope.** Unlike Propositions 1–3, which hold for any monotone convex penalty
function e(a) and do not require regulator optimality, P4 requires the regulator to
be setting the K&S second-best optimal schedule. The second-best is defined by the
regulator observing only k (violations), not θ (type) or a (negligence). The
first-best — defined by K&S eq. (10), h'[ã**(θ)] = θb'[ã**(θ)] — would require
assigning each type its socially efficient negligence level directly; P4 does not
characterize this. Mookherjee and Png (1994) do not state penalty convexity as a
formal proposition; their Propositions 1–2 characterize optimal marginal deterrence
and are the source for P1–P3. P4 is grounded in K&S eq. (7) and confirmed
empirically by K&S Fig. 2.

**Empirical implementation (Counterfactual 3).** At recovered structural parameters
(k_MR, 2-state machine), compute e*(a) from K&S eq. (7). Numerically invert the
Poisson mixture:

```
e(a) = Σ_{k=0}^∞  ε(k) · e^{−a} · a^k / k!
```

to recover the implied optimal discrete schedule ε*(k). Compare to the observed
SDWA MR enforcement schedule. Three possible findings: (i) observed schedule
subconvex — schedule redesign reduces violations without raising enforcement costs;
(ii) observed schedule matches ε*(k) — compliance gap driven by enforcement
intensity, not schedule shape; (iii) observed schedule superconvex — over-deters
repeat violators, under-deters marginal violators. CF3 and CF1 jointly decompose
the total compliance gap into schedule shape vs. enforcement intensity.
...
| P4 | optimal e*(a) | e*'(a) = θ(a)b'(a) > 0; e*''(a) > 0 when θ'(a)b'(a) > θ(a)\|b''(a)\| | K&S eq. (7) + Assumption 2 + strictly increasing a*(θ); confirmed K&S Fig. 2 |
...
A1 is also given normative grounding by Proposition 4: the K&S second-best optimal
schedule has e*'(a) = θ(a)b'(a) > 0, which via the Poisson structure means
E_a[ε(K+1) − ε(K)] > 0, confirming that A1 holds at the second-best optimum. In
the positive analysis (P1–P3) A1 is maintained; P4 shows the optimal schedule
satisfies it.
...
regardless of the sign of e''(a). If b is linear (b'' = 0), P4 directly provides
e*''(a) = θ'(a)b'(a) > 0, which is sufficient for the SOC without any additional
assumption on ε.
