# Session: 2026-04-14 — Pre-rule NaN encoding for VOC, SOC, and SWTR
...
- `run_main_tables.r`:
  ...
  - Policy: if IV fails, drop ALL columns for that outcome (user preference — do not show
    OLS/RF without IV).
...
- `surface_ground_water_rule_MCL_share_days` is a constant (zero) — no CWS in either
